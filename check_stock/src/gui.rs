...
impl StockCheckerApp {
    fn check_stock(&self) -> Result<String, Box<dyn std::error::Error>> {
        ...
        // Process inventory and wantslist
        ...
        if self.show_picking_list {
            Ok(d2d_automations::format_picking_list(&all_matching_cards))
        } else {
            Ok(d2d_automations::format_regular_output(&[(String::new(), all_matching_cards)]))
        }
    }
}
