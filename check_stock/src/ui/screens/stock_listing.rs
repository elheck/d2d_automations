...
use crate::ui::{
    state::{
        CardFetchMessage, CardFetchResult, FocusRequest, PriceGuideMessage, Screen,
        StockListingState,
    },
    style,
};
...
impl StockListingScreen {
    pub fn show(ctx: &egui::Context, current_screen: &mut Screen, state: &mut StockListingState) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            style::screen_heading(ui, "Magic Singles Listing");

            // ── Price guide ─────────────────────────────────────────────────
            style::section_frame().show(ui, |ui| {
                if state.price_guide.is_none() && !state.price_guide_loading {
                    ui.horizontal(|ui| {
                        if style::secondary_button(ui, "Load Cardmarket Prices").clicked() {
                            state.error = None;
                        ...
                        ui.label(
                            egui::RichText::new("Downloads ~50MB price data from Cardmarket")
                                .color(style::TEXT_MUTED)
                                .size(12.0),
                        );
                    });
                } else if state.price_guide_loading {
                    style::status_loading(ui, "⏳ Loading price guide...");
                } else if state.price_guide.is_some() {
                    style::status_ok(ui, "✓ Price guide loaded");
                }
            });
            ...
            style::section_frame().show(ui, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    if (qty_enter_pressed && can_fetch)
                        || style::primary_button_enabled(ui, "Fetch", can_fetch).clicked()
                    {
                    ...
                    if let Some((set, num)) = parsed {
                        ui.label(
                            egui::RichText::new(format!("→ {} #{}", set.to_uppercase(), num))
                                .color(style::ACCENT)
                                .size(13.0),
                        );
                    }
                ...
                if state.image_loading {
                    ui.add_space(6.0);
                    style::status_loading(ui, "⏳ Fetching card...");
                }
        ...
        style::section_frame().show(ui, |ui| {
            ui.horizontal(|ui| {
                ...
                if let Some(ref texture) = state.card_image {
                    ...
                    ui.label(egui::RichText::new("(No image available)").color(style::TEXT_MUTED));
                }
                ...
                ui.vertical(|ui| {
                    ...
                    // Scryfall prices
                    ui.label(
                        egui::RichText::new("Scryfall Prices:")
                            .strong()
                            .color(style::TEXT_PRIMARY),
                    );
                    ui.horizontal(|ui| {
                    ...
                    if let Some(cardmarket_id) = card.cardmarket_id {
                        ...
                        if let Some(ref guide) = state.price_guide {
                            if let Some(prices) = guide.get(cardmarket_id) {
                                ui.add_space(5.0);
                                ui.label(
                                    egui::RichText::new("Cardmarket Price Guide")
                                        .strong()
                                        .color(style::TEXT_PRIMARY),
                                );
                                ...
                                ui.label(
                                    egui::RichText::new("(Not found in price guide)")
                                        .color(style::TEXT_MUTED),
                                );
                            }
                        }
