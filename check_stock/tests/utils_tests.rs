...
fn test_path_exists_with_different_path_types() {
    ...
    assert!(path_exists(temp_path.to_path_buf())); // PathBuf
...
fn test_get_current_time_is_recent() {
    ...
    let parsed_time = DateTime::parse_from_str(&format!("{}+00:00", time_str), "%Y-%m-%d %H:%M:%S%z")
    ...
    assert!(diff.num_seconds().abs() < 5, 
            "Time difference too large: {} seconds", diff.num_seconds());
}
