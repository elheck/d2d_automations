...
use crate::models::{OrderRecord, CardRecord};
...
impl CsvProcessor {
    ...
    fn parse_csv_content(&self, content: &str) -> Result<Vec<OrderRecord>> {
        ...
        if header_line.contains("OrderID") || header_line.contains("Username") || header_line.contains("Name") {
    ...
    fn parse_csv_with_headers(&self, content: &str) -> Result<Vec<OrderRecord>> {
        ...
        info!("Processing {} data lines (excluding header)", data_lines.len());
    ...
    fn parse_order_line(&self, line: &str) -> Result<OrderRecord> {
        ...
        if parts.len() < 18 {
            let error_msg = format!("Invalid CSV format. Expected at least 18 columns, got {}", parts.len());
            warn!("{}", error_msg);
        ...
        let order_record = OrderRecord {
            ...
            is_professional: if parts[6].trim().is_empty() { None } else { Some(parts[6].trim().to_string()) },
            vat_number: if parts[7].trim().is_empty() { None } else { Some(parts[7].trim().to_string()) },
            date_of_purchase: parts[8].trim().to_string(),
            article_count: parts[9].trim().parse::<u32>()
    ...
    fn parse_city_field(&self, city_field: &str) -> Result<(String, String)> {
        ...
        if parts.len() < 2 {
            ...
            warn!("City field '{}' doesn't contain postal code, using as city name only", city_field);
    ...
    fn parse_card_line(&self, line: &str) -> Result<CardRecord> {
        ...
        let (set_name, collector_number, rarity, condition, language) = self.parse_set_info(&set_info);
        let currency = if price.contains("EUR") { "EUR" } else if price.contains("$") { "USD" } else { "EUR" };
    ...
    pub fn validate_orders(&self, orders: &[OrderRecord]) -> Vec<String> {
        ...
        for (index, order) in orders.iter().enumerate() {
            ...
            if order.street.trim().is_empty() {
                debug!("Line {}: Street address is empty (this may be acceptable)", line_num);
            }
            ...
            if order.total_value.trim().is_empty() {
                ...
                if normalized_price.parse::<f64>().is_err() {
                    let error_msg = format!("Line {}: Invalid total value format: {}", line_num, order.total_value);
                    warn!("{}", error_msg);
                    errors.push(error_msg);
                } else {
                    debug!("Line {}: Price format acceptable after normalization: {} -> {}", line_num, order.total_value, normalized_price);
                }
            }
